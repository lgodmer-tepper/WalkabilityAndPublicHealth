...
The objective is to determine the quantitative causal relationship between the "walkability" of a region and public health. 
...
Quantifying the causal effect of walkability and public health can help policy makers decide on community planning measures that may improve or degrade the walkability of the region, by helping them to understand the public health impact of such decisions.

The appendix of this document describes the pre-processing methodology to combine the two data sets to enable the quantitative analysis. Because the pre-processing methodology can take an hour or more to execute, the pre-processed data is exported and the rest of the analysis is done based on the pre-processed data.
...
All of the commands are commented out to prevent them from being executed on knit.
...
They are commented out here to avoid accidentally triggering a download. Un-comment these two lines and run them to download the raw data
